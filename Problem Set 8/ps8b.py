...
class Patient(object):
    ...
    def update(self):
        ...
        ## tests if each virus in the list survives:
        ...
        self.popDensity=float(len(self.getViruses()))/self.getMaxPop()
        
        ##determine whether each virus should reproduce; add new ones to list
        tempViruses=self.viruses[:]
        for virus in tempViruses:
            try:
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    ##setting up the time Step buckets
    popAtTime=[]
    timeSteps=300
    ...
    while trialNum<numTrials:
        ##setting up the number of viruses to start
        trialViruses=[]
        singleVirus=SimpleVirus(maxBirthProb,clearProb)
        for x in range(numViruses):
            trialViruses.append(singleVirus)
        ##creating the test patient
        testPatient=Patient(trialViruses,maxPop)
        for step in range(timeSteps):
            testPatient.update()
            popAtTime[step].append(testPatient.getTotalPop())

            ## print step
            
        trialNum+=1
        
    ##average out the populations from all the simulations
    avgPopAtTime=[]
    for group in popAtTime:
        avgPopAtTime.append(float(sum(group))/len(group))
    # print avgPopAtTime

    ##create the gorram plot
    pylab.figure(1)
    pylab.plot(avgPopAtTime)
    pylab.title('Average SimpleVirus Population over Time in 1 Patient')
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        ##random test for reproduction
        if random.random()<self.getMaxBirthProb()*(1 - popDensity):

            ##determining whether mutations will stay or switch
            tempResistances=self.getResistances()
            newResistances={}
            for drug in tempResistances:
                if random.random()<1-self.getMutProb():
                    newResistances[drug]=tempResistances[drug]
                else:
                    if tempResistances[drug]==True:
                        newResistances[drug]=False
                    elif tempResistances[drug]==False:
...
class TreatedPatient(Patient):
    ...
    def addPrescription(self, newDrug):
        ...
        if newDrug in self.drugsAdministered:
            pass
        else:
    ...
    def getResistPop(self, drugResist):
        ...
        for virus in self.viruses:
            resist=virus.getResistances()
            for drug in drugResist:
                try:
                    if resist[drug]==True:
                        if drugResist[-1]==drug:
                            resistPop+=1
                    else:
                        break
                except KeyError:
    ...
    def update(self):
        ...
        for virus in tempViruses:
            try:
                virus.reproduce(self.popDensity, self.drugsAdministered)
                self.viruses.append(virus)
            except NoChildException:
                pass
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    timeMiddle=150
    timeEnd=300
    for x in range(timeEnd):
        popAtTime.append([])
    for x in range(timeEnd):
        resistPopAtTime.append([])
    ...
    while trialNum<numTrials:
        ...
        for step in range(timeBegin, timeMiddle):
            popAtTime[step].append(testPatient.update())
            #print testPatient.getResistPop(['guttagonol'])
            resistPopAtTime[step].append(testPatient.getResistPop(['guttagonol']))
    ...
    # print avgPopAtTime
    avgResistPopAtTime=[]
    for group in resistPopAtTime:
        avgResistPopAtTime.append(float(sum(group))/len(group))
    ##create the gorram plot
    pylab.figure(1)
    pylab.plot(avgPopAtTime)
    pylab.plot(avgResistPopAtTime)
    pylab.title('Average Resistant Virus Population over Time')
    pylab.xlabel('Time Steps')
    pylab.ylabel('Average Virus Population')
