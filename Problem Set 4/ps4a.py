...
def isValidWord(word, hand, wordList):
    ...
    if wordLen>handLen:
        ...
        while x < (len(word)):
            if wordDict[word[x]] > hand.get(word[x], 0):
                return False
            else:
                x+=1
        return True
    ...
    while n>0:
        ...
        # Otherwise (the input is not a single period):
            ...
            # Otherwise (the word is valid):
                ...
                #wordScore depends on whether n is the original length of the hand
                if count==1 and len(word)==n:
                    wordScore=getWordScore(word,n)
                elif count>1 and len(word) == n:
                    wordScore=(getWordScore(word,n)-50)
                else:
                    wordScore=getWordScore(word,n)
                totalScore+=wordScore
                print '"'+str(word)+'" earned '+ str(wordScore) +'. Total: '+ str(totalScore)+ ' points.'
                print
                # Update the hand 
